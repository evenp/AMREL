 ...
 */
template<typename PixelType>
int write_2D_png_image(const Image2D<PixelType> &image, std::string outputPath){
 ...
 */
template<typename PixelType>
int write_2D_png_color_image(const Image2D<PixelType> &image,
    ...
    // Output is 24bit depth, RGB format.
    png_set_IHDR(
      png,
      info,
      width, height,
...
//            px[0] = image(x,y);
            px[0] = image(x,y) / (256 * 256);
            px[1] = (image(x,y) / 256) % 256;
            px[2] = image(x,y) % 256;
        }
    }
